...
% Import the data
simplelinearlownoise = readtable("../MATLAB/data/simple_cubic_highnoise_n201.csv", opts);
...
for i = 1:size(train_x, 2)
    prior.cov{i} = {@priorTransform,@exp,@exp,@log,{@priorGamma,2,3/2}}; % Gamma prior on length scales
end
prior.cov{(size(train_x, 2) + 1)} = {@priorTransform,@exp,@exp,@log,{@priorGamma,2,3/2}}; % Gamma prior on scale factor 
prior.lik = {{@priorTransform,@exp,@exp,@log,{@priorGamma,2,3/2}}}; % Gamma prior on noise
...
scatter(train_x, train_y4); title("Simple cubic DGP with high noise (n=201)");

fig = figure(3);
expchain = exp(chain);
plotmatrix(expchain); title("Posterior hypers: simple cubic DGP with high noise (n=201)")
