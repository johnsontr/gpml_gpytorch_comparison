...
opts.VariableNames = ["VarName1", "y2", "x"];
...
% Import the data
simplelinearlownoise = readtable("../MATLAB/data/simple_linear_highnoise_n201.csv", opts);
...
train_y2 = table2array(simplelinearlownoise(:,"y2"));
...
for i = 1:size(train_x, 2)
    prior.cov{i} = {@priorTransform,@exp,@exp,@log,{@priorGamma,1,1/5}}; % Gamma prior on length scales
end
prior.cov{(size(train_x, 2) + 1)} = {@priorTransform,@exp,@exp,@log,{@priorGamma,1,1/5}}; % Gamma prior on scale factor 
prior.lik = {{@priorTransform,@exp,@exp,@log,{@priorGamma,1,1/5}}}; % Gamma prior on noise
...
hypSEard = minimize_v2(hypSEard, @gp, p, inffunc, meanfunc, covfunc, likfunc, train_x, train_y2);

% Report negative log marginal likelihood of the data with the optimized model hyperparameters.
nlml2 = gp(hypSEard, inffunc, meanfunc, covfunc, likfunc, train_x, train_y2);

% Make predictions
predict_z = linspace(-2, 2, 101)'; % Grid on which to predict values
[m, s2] = gp(hypSEard, inffunc, meanfunc, covfunc, likfunc, train_x, train_y2, predict_z);
...
f = @(unwrapped_theta) customize_ll(unwrapped_theta, hypSEard_ind, hypSEard, inffunc, meanfunc, covfunc, train_x, train_y2);  
...
for i=1:size(chain,1)
    ...
    [~, ~, mu, s2] = gp(hypSEard_0, inffunc, meanfunc, covfunc, [], train_x, train_y2, predict_z);
    mus{i} = mu;
    s2s{i} = s2;
end
...
scatter(train_x, train_y2); title("Simple linear DGP with high noise (n=201)");

fig = figure(3);
expchain = exp(chain);
plotmatrix(expchain); title("Posterior hypers: simple linear DGP with high noise (n=201)")
