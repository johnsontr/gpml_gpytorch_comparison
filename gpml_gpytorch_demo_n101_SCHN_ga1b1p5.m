...
opts.VariableNames = ["VarName1", "y4", "x"];
...
% Import the data
simplelinearlownoise = readtable("../MATLAB/data/simple_cubic_highnoise_n101.csv", opts);
...
train_y4 = table2array(simplelinearlownoise(:,"y4"));
...
for i = 1:size(train_x, 2)
    prior.cov{i} = {@priorTransform,@exp,@exp,@log,{@priorGamma,1,3/2}}; % Gamma prior on length scales
end
prior.cov{(size(train_x, 2) + 1)} = {@priorTransform,@exp,@exp,@log,{@priorGamma,1,3/2}}; % Gamma prior on scale factor 
prior.lik = {{@priorTransform,@exp,@exp,@log,{@priorGamma,1,3/2}}}; % Gamma prior on noise
...
hypSEard = minimize_v2(hypSEard, @gp, p, inffunc, meanfunc, covfunc, likfunc, train_x, train_y4);

% Report negative log marginal likelihood of the data with the optimized model hyperparameters.
nlml2 = gp(hypSEard, inffunc, meanfunc, covfunc, likfunc, train_x, train_y4);

% Make predictions
predict_z = linspace(-2, 2, 101)'; % Grid on which to predict values
[m, s2] = gp(hypSEard, inffunc, meanfunc, covfunc, likfunc, train_x, train_y4, predict_z);
...
f = @(unwrapped_theta) customize_ll(unwrapped_theta, hypSEard_ind, hypSEard, inffunc, meanfunc, covfunc, train_x, train_y4);  
...
for i=1:size(chain,1)
    ...
    [~, ~, mu, s2] = gp(hypSEard_0, inffunc, meanfunc, covfunc, [], train_x, train_y4, predict_z);
    mus{i} = mu;
    s2s{i} = s2;
end
...
scatter(train_x, train_y4); title("Simple cubic DGP with high noise (n=101)");

fig = figure(3);
expchain = exp(chain);
plotmatrix(expchain); title("Posterior hypers: simple cubic DGP with high noise (n=101)")
