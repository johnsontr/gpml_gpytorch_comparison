...
for i = 1:size(train_x, 2)
    prior.cov{i} = {@priorTransform,@exp,@exp,@log,{@priorGamma,2,2}}; % Gamma prior on length scales
end
prior.cov{(size(train_x, 2) + 1)} = {@priorTransform,@exp,@exp,@log,{@priorGamma,2,2}}; % Gamma prior on scale factor 
prior.lik = {{@priorTransform,@exp,@exp,@log,{@priorGamma,2,2}}}; % Gamma prior on noise
