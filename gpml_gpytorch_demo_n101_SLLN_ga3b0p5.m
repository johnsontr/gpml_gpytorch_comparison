...
% Import the data
simplelinearlownoise = readtable("../MATLAB/data/simple_linear_lownoise_n101.csv", opts);
...
for i = 1:size(train_x, 2)
    prior.cov{i} = {@priorTransform,@exp,@exp,@log,{@priorGamma,3,1/2}}; % Gamma prior on length scales
end
prior.cov{(size(train_x, 2) + 1)} = {@priorTransform,@exp,@exp,@log,{@priorGamma,3,1/2}}; % Gamma prior on scale factor 
prior.lik = {{@priorTransform,@exp,@exp,@log,{@priorGamma,3,1/2}}}; % Gamma prior on noise
...
scatter(train_x, train_y1); title("Simple linear DGP with low noise (n=101)");

fig = figure(3);
expchain = exp(chain);
plotmatrix(expchain); title("Posterior hypers: simple linear DGP with low noise (n=101)")
