...
opts.VariableNames = ["VarName1", "y3", "x"];
...
% Import the data
simplelinearlownoise = readtable("../MATLAB/data/simple_cubic_lownoise_n201.csv", opts);
...
train_y3 = table2array(simplelinearlownoise(:,"y3"));
...
hypSEard = minimize_v2(hypSEard, @gp, p, inffunc, meanfunc, covfunc, likfunc, train_x, train_y3);

% Report negative log marginal likelihood of the data with the optimized model hyperparameters.
nlml2 = gp(hypSEard, inffunc, meanfunc, covfunc, likfunc, train_x, train_y3);

% Make predictions
predict_z = linspace(-2, 2, 101)'; % Grid on which to predict values
[m, s2] = gp(hypSEard, inffunc, meanfunc, covfunc, likfunc, train_x, train_y3, predict_z);
...
f = @(unwrapped_theta) customize_ll(unwrapped_theta, hypSEard_ind, hypSEard, inffunc, meanfunc, covfunc, train_x, train_y3);  
...
for i=1:size(chain,1)
    ...
    [~, ~, mu, s2] = gp(hypSEard_0, inffunc, meanfunc, covfunc, [], train_x, train_y3, predict_z);
    mus{i} = mu;
    s2s{i} = s2;
end
...
scatter(train_x, train_y3); title("Simple cubic DGP with low noise (n=201)");

fig = figure(3);
expchain = exp(chain);
plotmatrix(expchain); title("Posterior hypers: simple cubic DGP with low noise (n=201)")
